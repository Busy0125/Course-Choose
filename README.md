...
- **系统管理员**
  ...
  - 提供学生，教师，课程，班级信息的新增，删除，更新功能。其中四者的ID（数据库表中均为主键）需手动输入，且暂无判误（主键重复）功能（已完善）。新增班级时提供教师时间冲突提示功能。
...
- 4月20日更新
  ...
  - 新增选课时不能选择同一学期同一课程的警告，修复了选课时判断时间冲突有误的bug。

<br>

## 验收

- 先展示数据库。注意说明系统分为三种学生，教师，系统管理员三种角色，登录时会做出区分。
...
- 管理员登录
  ...
  - 头部菜单，提供修改密码，退出登录。
  - 课程的增删改，分页，根据学院筛选，模糊查询。新增时包括ID判误。
  - 学生的增删改，分页，根据学院筛选，模糊查询，更新绩点之后再介绍。新增时包括ID判误。
  - 教师的增删改，分页，根据学院筛选，模糊查询。新增时包括ID判误。
  - 班级的增删改，根据学院筛选，模糊查询。新增时包括ID判误。新增和更新时包括教师时间冲突警告。
  - 院系的增删改。
  - 各表可按部分字段排序。
...
- 学生登录
  ...
  - 选课，模糊查询。
  - 选课失败警告：人数已满，重复选课，同一学期重复选同一门，时间冲突。
  - 退课，模糊查询。
  - 退课失败警告：已考核就无法退课。
  - 课表。按学期筛选，可展示课表，记录数，当前总学分。
  - 成绩。按学期筛选，可展示各学期平均绩点。
  - 各表可按部分字段排序。
...
- 教师登录
  ...
  - 模糊查询。
  - 各班级分别成绩登录。
  - 编辑平时成绩，考试成绩后，会提示需更新综合成绩。此时无法正常退出。
  - 生成综合成绩输入不合法警告。
  - 查看成绩分布，平均分。
  - 回头去介绍系统管理员的更新平均绩点。
  - 各表可按部分字段排序。
...
- 存储过程和触发器
  - 系统管理员登录，各部门人数信息通过调用存储过程获取。
  - 触发器。删除学生时删除相应成绩。删除班级时删除相应成绩（选课记录）。删除课程时删除相应班级和成绩。删除部门时删除学生，教师，课程，班级，成绩。
